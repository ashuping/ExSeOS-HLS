...
def test_warnings():
	assert ComparableError.array_encapsulate(Warning([ArithmeticError(6)], 37).warnings) == [ArithmeticError(6)]
	assert ComparableError.array_encapsulate(Error([SyntaxError()], [ArithmeticError(6)]).warnings) == [ArithmeticError(6)]
	
	with raises(TypeError):
		Okay(37).warnings

def test_errors():
	assert ComparableError.array_encapsulate(Error([SyntaxError()], [ArithmeticError(6)]).errors) == [SyntaxError()]
...
def test_eq():
	...
	assert Error([SyntaxError()], [ArithmeticError(6)]) == Error([SyntaxError()], [ArithmeticError(6)])
...
def test_okay_rshift():
	...
	assert Okay(37) >> Warning([ArithmeticError(6)], 36)                  == Warning([ArithmeticError(6)], 36)
	assert Okay(37) >> Error([SyntaxError("test")], [ArithmeticError(6)]) == Error([SyntaxError("test")], [ArithmeticError(6)])

def test_warning_rshift():
	assert Warning([ArithmeticError(6)], 37) \
		>> Okay(36) \
		== Warning([ArithmeticError(6)], 36)

	assert Warning([ArithmeticError(6)], 37) \
		>> Warning([SyntaxError("test")], 36) \
		== Warning([ArithmeticError(6), SyntaxError("test")], 36)

	assert Warning([ArithmeticError(6), ArithmeticError(7)], 36) \
		>> Warning([SyntaxError("test"), TypeError("cool")], 37) \
		== Warning([ArithmeticError(6), ArithmeticError(7), SyntaxError("test"), TypeError("cool")], 37)

	assert Warning([ArithmeticError(6)], 37) \
		>> Error([SyntaxError("test")], [TypeError("cool")]) \
		== Error([SyntaxError("test")], [ArithmeticError(6), TypeError("cool")])

def test_error_rshift():
	assert Error([SyntaxError("test")], [ArithmeticError(6)]) \
		>> Okay(36) \
		== Error([SyntaxError("test")], [ArithmeticError(6)])

	assert Error([SyntaxError("test")], [ArithmeticError(6)]) \
		>> Warning([TypeError("cool")], 37) \
		== Error([SyntaxError("test")], [ArithmeticError(6), TypeError("cool")])

	assert Error([SyntaxError("test")], [ArithmeticError(6)]) \
		>> Error([OSError("wow")], [TypeError("cool")]) \
		== Error([SyntaxError("test"), OSError("wow")], [ArithmeticError(6), TypeError("cool")])
	
	assert Error([SyntaxError("test"), SyntaxError("test2")], [ArithmeticError(6), ArithmeticError(7)]) \
		>> Error([OSError("wow")], [TypeError("cool")]) \
		== Error(
...
def test_warning_map():
	assert Warning([ArithmeticError(6)], 37).map(lambda x: f"my {x}") \
		== Warning([ArithmeticError(6)], "my 37")

def test_error_map():
	assert Error([ArithmeticError(6)], [TypeError("test")]).map(lambda x: f"my {x}") \
		== Error([ArithmeticError(6)], [TypeError("test")])

def test_okay_flatmap():
	assert Okay(37).flat_map(lambda x: Okay(f"my {x}")) \
		== Okay("my 37")

	assert Okay(37).flat_map(lambda x: Warning([ArithmeticError(6)], f"my {x}")) \
		== Warning([ArithmeticError(6)], f"my 37")

	assert Okay(37).flat_map(lambda x: Error([TypeError("test")], [ArithmeticError(6)])) \
...
def test_flatmap_short_circuit_on_error():
	res = Okay(37).flat_map(
		lambda x: Okay(f"precious {x}")
	).flat_map(
		lambda x: Error([SyntaxError(f"cool {x}")], [TypeError("test")])
	).flat_map(
		lambda x: Error([SyntaxError(f"wow, {x}")], [TypeError("test2")])
	) 
